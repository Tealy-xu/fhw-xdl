...
<body>
<div class="pd-20">
	<form action="/lamp/Fanhuawei/index.php/Admin/Admin/doRoleAddAdmin" method="post" class="form form-horizontal" id="form-user-character-add">
		<div class="row cl">
		...
		<div class="row cl">
			...
			<?php if(is_array($pauth_info)): $i = 0; $__LIST__ = $pauth_info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li><input type="checkbox" name="authname[]"  value="<?php echo ($v["auth_id"]); ?>"><?php echo ($v["auth_name"]); ?></li>
				<ul>

					<?php if(is_array($sauth_info)): foreach($sauth_info as $key=>$vv): if($vv['auth_pid'] == $v['auth_id']): ?><li>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="authname[]"   value="<?php echo ($vv["auth_id"]); ?>"><?php echo ($vv["auth_name"]); ?></li>
						
						<ul>
						<?php if(is_array($tauth_info)): foreach($tauth_info as $key=>$vvv): if($vvv['auth_pid'] == $vv['auth_id']): ?><li>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="authname[]"   value="<?php echo ($vvv["auth_id"]); ?>"><?php echo ($vvv["auth_name"]); ?></li><?php endif; endforeach; endif; ?>
